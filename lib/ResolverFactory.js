...
exports.createResolver = function (options) {
	...
	resolver.ensureHook("continueRawModule");
	...
	modules.forEach(item => {
		if (Array.isArray(item)) {
			if (item.includes("node_modules") && pnpApi) {
				...
				// When PnpPlugin can't resolve a module, continue looking in node_modules
				plugins.push(new NextPlugin("after-raw-module", "continue-raw-module"));
				plugins.push(
					new ModulesInHierachicDirectoriesPlugin(
						"continue-raw-module",
						item.filter(i => i === "node_modules"),
